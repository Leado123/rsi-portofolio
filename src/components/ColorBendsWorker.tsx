...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    // Create worker for time calculations (optional optimization)
    // Worker is optional - component works fine without it
    // Initialize worker asynchronously to not block rendering
    setTimeout(() => {
      if (typeof Worker !== 'undefined') {
        try {
          const worker = new Worker('/colorBendsWorker.js');
          worker.onmessage = (e) => {
            if (e.data && e.data.type === 'tick') {
              timeRef.current = e.data.elapsed;
            }
          };
          ...
          // Worker not available, will use main thread time calculation
          workerRef.current = null;
        }
      }
    }, 0);
    ...
    const loop = () => {
      ...
      // Use worker time if available and valid, otherwise use clock
      if (workerRef.current && timeRef.current > 0) {
